...
class IssueEntry extends StatefulWidget {
  const IssueEntry({super.key});

  @override
  State<IssueEntry> createState() => _IssueEntryState();
}

class _IssueEntryState extends State<IssueEntry> implements FilterView {
              ...
              itemBuilder: (BuildContext context, i) {
                ...
                return InkWell(
                  onTap: (() {
                    ...
                    Get.toNamed('/issue_list', arguments: data);
  ...
  void onGetFilterSuccess(FilterResponse data) {
    ...
    if (data.success!) {
      ...
      setState(() {
        if(list.length == 1){
          Get.toNamed('/issue_list', arguments: list[0]);
        }
      });
    } else {
      page--;
